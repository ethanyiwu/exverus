...
def find_while_loops(func_body_inner: str):
    ...
    while True:
        ...
        if inv_match:
            ...
            # Remove decreases from inv parsing area if present
            # Match decreases clause that may or may not end with comma
...
def build_tuple_rebind_line(indent: str, param_name_types, let_names, label_tag: str):
    # Include all parameters and let variables, excluding only reserved identifier 'mut'
    # For &mut Vec types, use vec![1] instead of the original variable name
    left_names = []
    right_values = []
    seen = set()
    ...
    for name, typ in param_name_types:
        ...
        if name not in seen:
            seen.add(name)
            left_names.append(f"mut {name}")
            # Check if it's a &mut Vec type
            if '&mut' in typ and 'Vec<' in typ:
                right_values.append("vec![1]")
            else:
                right_values.append(name)
...
def replace_while_with_assume_assert(func_inner: str, wl, param_name_types, let_names, label_tag: str, use_assert_for_loop_condition: bool = False) -> str:
    ...
    assumes = []
    # Loop condition
    assumes.append(f"{while_indent}// Loop condition")
    if use_assert_for_loop_condition:
        assumes.append(f"{while_indent}assert({wl['condition']});")
    else:
        assumes.append(f"{while_indent}assume({wl['condition']});")
    
    # Invariants before the loop
    if wl['invariants']:
        assumes.append(f"{while_indent}// Invariants before the loop")
        for inv in wl['invariants']:
            ...
            if inv_clean:
                if use_assert_for_loop_condition:
                    assumes.append(f"{while_indent}assert({inv_clean});")
                else:
    ...
    # Generate asserts from invariants (after the loop)
    asserts = []
    if wl['invariants']:
        asserts.append(f"{while_indent}// Invariants after the loop")
        for inv in wl['invariants']:
            inv_clean = strip_inline_comments(inv)
            # Skip malformed quantifiers like 'forall|k: int|' with no body
            if re.match(r"^forall\s*\|[^|]+\|\s*$", inv_clean):
...
def convert_file(input_path: Path, output_dir: Path, use_assert_for_loop_condition: bool = False) -> Path:
    ...
    for fn in fn_infos:
        ...
        for idx, wl in enumerate(whiles):
            ...
            label_tag = f"[{idx+1}]"
            new_inner = replace_while_with_assume_assert(inner, wl, param_name_types, let_names, label_tag, use_assert_for_loop_condition)
...
def main():
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print("Usage: python convert_assume.py path/to/input.rs path/to/save/folder [--use-assert]", file=sys.stderr)
        print("  --use-assert: Use 'assert' instead of 'assume' for loop conditions and invariants", file=sys.stderr)
        sys.exit(1)
    
    input_path = Path(sys.argv[1]).resolve()
    output_dir = Path(sys.argv[2]).resolve()
    use_assert = False  # Default to assume
    
    if len(sys.argv) == 4 and sys.argv[3] == "--use-assert":
        use_assert = True
    ...
    if not input_path.exists():
        ...
        sys.exit(2)
    try:
        out = convert_file(input_path, output_dir, use_assert)
