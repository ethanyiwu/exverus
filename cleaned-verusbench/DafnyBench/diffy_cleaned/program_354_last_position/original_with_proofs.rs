...
fn last_position(arr: &[int], elem: int) -> (pos: i32)
    ...
    ensures
        pos == -1 || (0 <= pos && pos < arr.len() as i32 && arr[pos as int] == elem && (pos == (arr.len() as i32 - 1) || arr[(pos + 1) as int] > elem)),
        forall|i: int| 0 <= i < arr.len() as int ==> arr[i] == arr[i],
{
    ...
    for i in 0..arr.len()
        invariant
            0 <= i && i <= arr.len(),
            pos == -1 || (0 <= pos && pos < i as i32 && arr[pos as int] == elem && (pos == (i - 1) as i32 || i == 0 || arr[(pos + 1) as int] > elem)),
            arr.len() > 0,
            arr.len() < i32::MAX as usize,
            forall|i: int, j: int| 0 <= i < j && j < arr.len() as int ==> arr[i] <= arr[j],
    {
