...
# [doc = " Specification function to check if an array is sorted"]
spec fn sorted(a: Seq<int>) -> bool {
    forall|j: int, k: int| 0 <= j < k < a.len() ==> a[j] <= a[k]
}

# [doc = " Specification function to check if an array is distinct"]
spec fn distinct(arr: Seq<int>) -> bool {
    forall|i: int, j: int| 0 <= i < arr.len() && 0 <= j < arr.len() ==> i == j ==> arr[i] != arr[j]
}

# [doc = " Specification function to check if a target is not found in an array"]
spec fn not_found(arr: Seq<int>, target: int) -> bool {
    forall|j: int| 0 <= j < arr.len() ==> arr[j] != target
}

# [doc = " Specification function to check if a target is found in an array"]
...
# [doc = " Function to perform binary search"]
